...
GetOptions('user_id=i' => \$user_id, 'pfam_id=i' => \$family_id);
unless ($family_id && $family_id ) {die "Usage: perl comm_anno_pic_gen.pl --user_id --pfam_id\n";}
...
for my $locus (@locus_info){
	...
	if ($locus->genbank_cdna_acc){
		...
		for my $acc (@accs) {
			my $link = "<a href=\"http://www.ncbi.nlm.nih.gov/entrez/viewer.fcgi?db=nucleotide&val=$acc\">$acc</a>, ";
			$cdna_link.=$link;
		}
		if (scalar(@accs) == 1){
			$cdna_link =~ s/,//;
	...
	my $protein_link;
		...
		for my $acc (@accs) {
			my $link = "<a href=\"http://www.ncbi.nlm.nih.gov/entrez/viewer.fcgi?db=protein&val=$acc\">$acc</a>, ";
			$protein_link.=$link;
		}
		if (scalar(@accs) == 1){
			$protein_link =~ s/,//;
