...
#Config
my $PROTEOME_BLAST_DB = '/export/wwwdocs1/docs/tigr-scripts/osa1/ca_devel/blast_dbs/rice_v5.pep';
my $WEBSERVER_TEMP_DIR = '/export/wwwdocs1/docs/webserver_tmp/';
...
my $APACHE_DOC_ROOT = '/export/wwwdocs1/docs/';
my $WEBSERVER_TEMP_REL = '/webserver_tmp';

#need this dbh for CGI::Session
my $dbh = DBI->connect( $CA_DB_DSN, $CA_DB_USERNAME, $CA_DB_PASSWORD ) or die( "cannot connect to CA database:$!");
...
sub select_family {
    ...
    for my $fam ( @fams ) {
        my $row = { family_id => $fam->family_id , name => $fam->name , description => $fam->description };
        push(@$gene_family_radio, $row);
    }
    $tmpl->param( gene_family_radio => $gene_family_radio,
                  name => $user->name,
...
sub annotate {
    ...
    my $anno_ref = $session->param('anno_ref');
    my $user = CA::users->retrieve( $session->param('user_id') );
    ...
    if ( $action  eq 'annotate' ) {
        ...
        for my $locus ( @annotated_loci ) {
            ...
            $anno_ref->{loci}->{$locus_id}->{has_structural_annotation} = $locus->has_structural_annotation;
    ...
    if ( $action  ne 'annotate' ) {
        for my $locus_id ( keys %{$anno_ref->{loci}} ) {
            ...
            $anno_ref->{loci}->{$locus_id}->{alt_gene_name} =  $cgi->param('alt_gene_name_'.$locus_id);
            $anno_ref->{loci}->{$locus_id}->{gene_description} = $cgi->param('gene_description_'.$locus_id);
            $anno_ref->{loci}->{$locus_id}->{genbank_genomic_acc} = $cgi->param('genbank_genomic_acc_'.$locus_id);
            $anno_ref->{loci}->{$locus_id}->{genbank_cdna_acc} = $cgi->param('genbank_cdna_acc_'.$locus_id);
            $anno_ref->{loci}->{$locus_id}->{genbank_protein_acc} = $cgi->param('genbank_protein_acc_'.$locus_id);
    ...
    if ($save) {
        for my $locus_id ( keys %{$anno_ref->{loci}} ) {
            ...
            $locus_obj->set( gene_name => $anno_ref->{loci}->{$locus_id}->{gene_name},
                             ...
                             has_structural_annotation =>  $anno_ref->{loci}->{$locus_id}->{has_structural_annotation},
                           );
            $locus_obj->update;
        }
    }
    ...
    for my $locus_id ( keys(%{$anno_ref->{loci}})) {
        ...
        $row->{has_structural_annotation} = $anno_ref->{loci}->{$locus_id}->{has_structural_annotation};
        push(@$annotation_loop, $row);
    }
    
    if ( $blast_results) {
        $tmpl->param( blast_results => 1,
...
sub run_blast {
    ...
    ###### set up temp files##############
    my $blast_out_fh = File::Temp->new( TEMPLATE => 'tempXXXXXXXXX',
                                        DIR => $WEBSERVER_TEMP_DIR,
                                        SUFFIX => '.blast',
                                        UNLINK => 1,
                                      ) or die;

    my $fasta_out_fh = File::Temp->new( TEMPLATE => 'tempXXXXXXXXX',
                                        DIR => $WEBSERVER_TEMP_DIR,
                                        SUFFIX => '.fasta',
                                        UNLINK => 1,
                                      ) or die;
    ...
    my $seqio_in = Bio::SeqIO->new( -fh => $string_fh,
                                    -format =>'fasta',
                                  );
    my $seqio_out = Bio::SeqIO->new ( -fh => $fasta_out_fh, 
                                      -format =>'fasta',
                                    );
    
    $seqio_out->write_seq($seqio_in->next_seq);
                  
    system("$BLASTALL -p $blast_program -d $PROTEOME_BLAST_DB -i ".$fasta_out_fh->filename." -e $evalue  -v 20 -b 20 -o ".$blast_out_fh->filename );
    
    my $blast_parser = Bio::SearchIO->new ( -format => 'blast',
...
sub add_loci {
    ...
    for my $locus ( @new_loci ) {
        my ($locus_obj) =  CA::loci->search( locus_name => $locus,  user_id => $anno_ref->{user_id}, family_id => $anno_ref->{family_id} );
...
sub structural_annotation {
    ...
    my ( $struct_locus_obj ) = CA::loci->search( user_id => $struct_anno_ref->{user_id}, family_id => $struct_anno_ref->{family_id}, locus_name => $locus  );
    ...
    my $ca_model_ds; 
    if (! $ca_model_json ) {    #JSON not passed as a paramtter
    ...
    if (! $ca_model_json ) { #no model JSON as a parameter or in the db
        ...
        $ca_model_ds = $json_handler->jsonToObj($ca_model_json);
    }
    my ($ca_model_feature) = create_ca_model_feature($ca_model_ds);
    my ( $url, $map, $map_name ) = create_ca_image_and_map( $gff_dbh, $locus_obj, $gene_models, $ca_model_feature );
...
sub submit_structural_annotation {
    ...
    if ($model_obj) { 
        ...
        $model_obj = CA::structural_annotation->insert({locus_id =>  $struct_locus_id,  model => $ca_model_json,  });
    }
...
sub generate_initial_ca_model_ds {
    ...
    my $json_handler = JSON->new;
    my $comm_anno_model_json =  $json_handler->objToJson($comm_anno_ds);
...
sub add_js_event_to_map {
    ...
    while (<$string_io>) {
        ...
        if ($line =~ /^<area/) {
            ...
            $line =~ s/href="\d+-\d+"/onmouseover="displayCoords($start, $stop, $rel_end5, $rel_end3);setCursor();" onmouseout="restoreCursor()"/;
        }
        $new_map .= $line;
    }
...
sub get_annotation_db_features {
    ...
    my @gene_models = $seg->features('processed_transcript:TIGR_annot_DB', -attributes => { 'Gene' => $locus} );
...
sub create_ca_image_and_map {
    ...
    my $panel = Bio::Graphics::Panel->new(
                                          ...
                                          -length => $ca_model_feature->length,
                                          -key_style => 'between',
                                          -width     => 600,
                                          -pad_left  => 20,
                                          -pad_right => 20,
    ...
    $panel->add_track( arrow => Bio::SeqFeature::Generic->new(-start=>$end5,
                                                              -end=>$end3),
                       -bump => 0,
                       -double=>1,
                       -tick => 2,
    ...
    $panel->add_track( $gene_models,
                       -glyph        => 'processed_transcript',
                       -connector    => 'solid',
                       -label        => 1,
                       -height       => 10,
                       -key          => 'TIGR Rice Gene Models',
                       -utr_color    => 'white',
                       -thin_utr     => 0,
                       -fgcolor      => 'slateblue',
                       -bgcolor      => 'skyblue',
    ...
    $panel->add_track($ca_model_feature, 
                      -glyph => 'processed_transcript',
                      -connector    => 'solid',
