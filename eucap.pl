...
sub init {
    ...
    my $hash = $user->hash;
    my $crypt_obj =
      Authen::Passphrase::MD5Crypt->new( salt => $salt, hash_base64 => $hash );
    ...
    while ( my $fam = $fams->next ) {
        ...
        if ( not defined $anno_ref->{is_admin} ) {
            ...
            $anno_ref->{family}->{$family_id}->{family_name} =
              $fam->family_name;
            $anno_ref->{family}->{$family_id}->{gene_class_symbol} =
              $fam->gene_class_symbol;
            $anno_ref->{family}->{$family_id}->{description} =
              $fam->description;
        }
    }
...
sub edit_profile {
    ...
    if ( $action eq "update_profile" ) {
        ...
        if ($new_photo) {
                ...
                and $extension =~ /\.[jpg|jpeg|png]/i )
            {
                ...
                my ( $width, $height ) =
                  imgsize("$WEBSERVER_TEMP_DIR/$new_photo_file_name");
    ...
    $jcvi_vars->{top_menu} = [
        ...
        },
        {
            'link'      => '/cgi-bin/medicago/eucap2/eucap.pl?action=logout',
            'menu_name' => 'Logout (<em>'
              . $anno_ref->{user}->{$user_id}->{username}
              . '</em>)'
        }
    ];
    $jcvi_vars->{main_content} = $tmpl->output;
...
sub annotate {
    ...
    my ( $user_id, $family_id ) =
      ( $cgi->param('user_id'), $cgi->param('family_id') );
    if ( defined $anno_ref->{is_admin} ) {
        &get_user_info( $anno_ref, $user_id );
    ...
    my $username =
      ( defined $anno_ref->{is_admin} ) ? "admin" : $user->username;
...
# count the number of loci for this family (uniq of all loci in the original & edits table
    ...
    foreach my $locus_id ( sort { $a <=> $b } keys %all_loci ) {
        ...
        if ( defined $locus_edits_obj ) {
            ...
            if ( $edits eq "" )
            {    # gene has been deleted if $edits exists and is empty
        ...
        if ( defined $anno_ref->{loci}->{$locus_id}->{mutant_id} and
            $anno_ref->{loci}->{$locus_id}->{mutant_id} ne "" ) {
            ...
            if ( defined $mutant_edits_obj ) {
                ...
                if ( $edits eq "" )
                {    # mutant info has been deleted. unassociate with locus
            ...
            my @annotated_alleles =
              CA::alleles->search( mutant_id => $mutant_id );
            my @edited_alleles =
              CA::alleles_edits->search( mutant_id => $mutant_id );

            my %all_alleles = ();
            $all_alleles{ $_->allele_id } = 1
              foreach ( ( @annotated_alleles, @edited_alleles ) );
            $anno_ref->{mutant}->{$mutant_id}->{has_alleles} =
              scalar keys %all_alleles;

            my $mutant_class_id =
              $anno_ref->{mutant}->{$mutant_id}->{mutant_class_id};
            my $mutant_class_edits_obj =
              CA::mutant_class_edits->retrieve($mutant_class_id);
            ...
            if ( defined $mutant_class_edits_obj ) {
                ...
                my $mutant_class_obj =
                  CA::mutant_class->retrieve($mutant_class_id);
                $mutant_class_hashref =
                  caObj_to_hashref( $mutant_class_obj, 'mutant_class' )
    ...
    for my $locus_id (@locus_ids) {
        ...
        $summary_row->{gene_locus} =
          $anno_ref->{loci}->{$locus_id}->{gene_locus};
        $summary_row->{orig_func_annotation} =
          $anno_ref->{loci}->{$locus_id}->{orig_func_annotation};
        $summary_row->{gene_symbol} =
          $anno_ref->{loci}->{$locus_id}->{gene_symbol};
        $summary_row->{func_annotation} =
          $anno_ref->{loci}->{$locus_id}->{func_annotation};
        $summary_row->{comment} = $anno_ref->{loci}->{$locus_id}->{comment};
        $summary_row->{tableRowClass} =
    ...
    $jcvi_vars->{page_header} =
      'Community Annotation for ' . $family->family_name . ' Gene Family';
    $jcvi_vars->{top_menu} = [
        {
            'link' => '/cgi-bin/medicago/eucap2/eucap.pl?action=select_family',
            'menu_name' => 'Select Family'
       ...
       #next unless ($gene_locus =~ /^Medtr[1-8]g\d+|^\S+_\d+|^contig_\d+_\d+/);
        ...
        if ( defined $locus_obj ) {
            ...
            while ( my $locus_edits_obj = $locus_edits_objs->next() ) {
                my $locus_edits_hashref =
                  JSON::from_json( $locus_edits_obj->edits );
        ...
        if ($locus_id) {
            ...
            until ( not defined $check ) {
                my $edits_max_locus_id =
                  &get_max_id( 'locus_id', 'v2_loci_edits' );
        ...
        $anno_ref->{loci}->{$locus_id}->{mod_date}             = q{};
...
sub annotate_locus {
    ...
    if ($save) {
        ...
        my $locus_hashref = {};
        if ( defined CA::loci_edits->retrieve($locus_id) ) {
            ...
            my $curr_locus_obj = CA::loci->retrieve($locus_id);
            $locus_hashref = caObj_to_hashref( $curr_locus_obj, 'loci' );
        }
        my $locus_edits_hashref = cgi_to_hashref( $cgi, 'loci' );
        ...
        # if true: get mutant_id from param or max(mutant_id) + 1 from db or increment it if it already exists in the edits table
            ...
            and $cgi->param('mutant_reference_pub') ne "" )
        {
            ...
            if ( $cgi->param('mutant_id') ne "" ) {
                ...
                $mutant_edits_hashref = cgi_to_hashref( $cgi, 'mutant_info' );

                my @differences =
                  data_diff( $mutant_hashref, $mutant_edits_hashref );
                ...
                my $max_mutant_id =
                  &get_max_id( 'mutant_id', 'v2_mutant_info' );
                $mutant_id = $max_mutant_id + 1;

                my $check = CA::mutant_info_edits->retrieve($mutant_id);
                until ( not defined $check ) {
                    my $edits_max_mutant_id =
                      &get_max_id( 'mutant_id', 'v2_mutant_info_edits' );
            ...
            if ( $cgi->param('mutant_class_id') ne "" ) {
                ...
                if (
                    defined CA::mutant_class_edits->retrieve($mutant_class_id) )
                {
                    my $mutant_class_obj =
                      CA::mutant_class_edits->retrieve($mutant_class_id);
                    $mutant_class_hashref = caObj_to_hashref( $mutant_class_obj, 'mutant_classs');
                }
                else {
                    my $mutant_class_obj =
                      CA::mutant_class->retrieve($mutant_class_id);
                    $mutant_class_hashref =
                      caObj_to_hashref( $mutant_class_obj, 'mutant_class' ) if (defined $mutant_class_obj);
                }

                $mutant_class_edits_hashref =
                  cgi_to_hashref( $cgi, 'mutant_class' );

                my @differences = data_diff( $mutant_class_hashref,
                    $mutant_class_edits_hashref );
                ...
                if (
                    defined CA::mutant_class_edits->search(symbol => $mutant_class_symbol) )
                {
                    my $mutant_class_obj =
                ...
                $mutant_class_id = $mutant_class_hashref->{mutant_id} if(defined $mutant_class_hashref);

                if(!$mutant_class_id) {
                    my $max_mutant_class_id =
                      &get_max_id( 'mutant_class_id', 'v2_mutant_class' );
            ...
            if ( $tmp_mutant_id ne undef ) {
                ...
                my $tmp_mutant_class_id =
                  $anno_ref->{mutant}->{$tmp_mutant_id}->{mutant_class_id};
                ...
                $mutant_edits_obj = CA::mutant_info_edits->insert(
                    {
                        mutant_id => $mutant_id,
                        edits =>
                          JSON::to_json( $anno_ref->{mutant}->{$mutant_id} )
                    }
                );
            }
        ...
        (
          ...
          ? print
          "Update success! Changes submitted for administrator approval."
            ...
            $anno_ref->{loci}->{$locus_id}->{mutant_id} ne "" )
        {
            ...
            $row->{mapping_data} =
              $anno_ref->{mutant}->{$mutant_id}->{mapping_data};
            $row->{mutant_reference_lab} =
              $anno_ref->{mutant}->{$mutant_id}->{reference_lab};
            $row->{mutant_reference_pub} =
              $anno_ref->{mutant}->{$mutant_id}->{reference_pub};
            $row->{mutant_mod_date} =
              $anno_ref->{mutant}->{$mutant_id}->{mod_date};
...
sub delete_locus {
    ...
    if ( defined $locus_edits_obj ) {
        if ( defined $locus_obj )
        {    # If locus_obj exists, just empty the 'edits' column
...
sub add_alleles {
  ...
  ALLELE: for my $allele_name (@new_alleles) {
            ...
            && exists $anno_ref->{allele}->{ $allele_obj->allele_id } )
        {
            ...
            while ( my $allele_edits_obj = $allele_edits_objs->next() ) {
                my $allele_edits_hashref =
                  JSON::from_json( $allele_edits_obj->edits );
        ...
        if ($allele_id) {
            ...
            until ( not defined $check ) {
                my $edits_max_allele_id =
                  &get_max_id( 'allele_id', 'v2_alleles_edits' );
    ...
    #annotate($session, $cgi);
    print $track, ' allele', ( $track > 2 or $track == 0 ) ? 's' : '',
      ' added!';
}
...
sub annotate_alleles {
    my ( $session, $cgi, $save ) = @_;
    my $tmpl =
      HTML::Template->new( filename => "./tmpl/annotate_alleles.tmpl" );
    my $anno_ref  = $session->param('anno_ref');
    my $user      = CA::users->retrieve( $anno_ref->{user_id} );
    my $mutant_id = $cgi->param('mutant_id');
    ...
    if ($save) {
        for my $allele_id ( keys %{ $anno_ref->{allele} } ) {
            ...
            if ( defined CA::alleles_edits->retrieve($allele_id) ) {
                ...
                $allele_hashref =
                  caObj_to_hashref( $curr_allele_obj, 'alleles' );
            }

            my $allele_edits_hashref =
              cgi_to_hashref( $cgi, 'alleles', $allele_id );

            my @differences =
              data_diff( $allele_hashref, $allele_edits_hashref );
            ...
            $anno_ref->{allele}->{$allele_id} = ( $track )
        ...
        $mutant_symbol = fix_missing_mutant_symbol( $mutant_symbol, $mutant_class_id ) if ( $mutant_obj->symbol eq "-" or $mutant_obj->symbol eq "" );
        ...
        for my $allele_id (@allele_ids) {
            ...
            $row->{allele_name} =
              $anno_ref->{allele}->{$allele_id}->{allele_name};
            $row->{alt_allele_names} =
              $anno_ref->{allele}->{$allele_id}->{alt_allele_names};
            $row->{reference_lab} =
              $anno_ref->{allele}->{$allele_id}->{reference_lab};
            $row->{altered_phenotype} =
              $anno_ref->{allele}->{$allele_id}->{altered_phenotype};
...
sub delete_allele {
    ...
    if ( defined $allele_edits_obj ) {
        if ( defined $allele_obj )
        {    # If allele_obj exists, just empty the 'edits' column
...
sub structural_annotation {
    ...
    my $tmpl =
      HTML::Template->new( filename => "./tmpl/structural_annotation.tmpl" );

    my ( $gff_locus_obj, $gene_models ) =
      get_annotation_db_features( $gene_locus, $gff_dbh );
 ...
 #when hooked into script - look for saved JSON in table if none passed as param
    ...
    if ( !$ca_model_json ) {    #JSON not passed as a parameter
        $sa_object =
          CA::structural_annot_edits->retrieve( locus_id => $locus_id );
    ...
    if ( !$ca_model_json ) {    #no model JSON as a parameter or in the db
        ( $ca_model_ds, $ca_model_json ) =
          generate_initial_ca_model_ds( $gene_models->[0] );
    }
    else {
    ...
    my ( $url, $map, $map_name ) =
      create_ca_image_and_map( $gff_dbh, $gff_locus_obj, $gene_models,
        $ca_model_feature );
...
sub review_annotation {
    ...
    my $tmpl =
      HTML::Template->new( filename => "./tmpl/review_annotation.tmpl" );
    my $anno_ref = $session->param('anno_ref');
    my $user     = CA::users->retrieve( $anno_ref->{user_id} );
...
sub submit_annotation {
    my ( $session, $cgi ) = @_;
    my $tmpl =
      HTML::Template->new( filename => "./tmpl/submit_annotation.tmpl" );
    my $anno_ref = $session->param('anno_ref');
    my $user     = CA::users->retrieve( $anno_ref->{user_id} );
    my $family   = CA::family->retrieve( $anno_ref->{family_id} );
...
s/\s+chr\d{1}\s+\d+\-\d+\s+.*|\s+contig_\d+\s+\d+\-\d+\s+.*|\s+\w{2}\d+\.\d+\s+\d+\-\d+\s+.*//gs;
        ...
        $blast_results->{hits}->{$hit_count}->{hit_name} =
          filter_hit_name( $hit->name );
    ...
    if ($blast_results) {
        ...
        for my $hit ( keys %{ $blast_results->{hits} } ) {
            ...
            $row->{description} =
              $blast_results->{hits}->{$hit}->{hit_description};
...
sub get_family_info {
    ...
    $anno_ref->{family}->{$family_id}->{gene_class_symbol} =
      $family_obj->{gene_class_symbol};
    $anno_ref->{family}->{$family_id}->{description} =
      $family_obj->{description};
...
sub get_user_info {
    ...
    $anno_ref->{user}->{$user_id}->{photo_file_name} =
      $user_obj->photo_file_name;
...
sub caObj_to_hashref {
    ...
    my %table_columns = (
        ...
        'mutant_info' => [
            'symbol',        'phenotype',
            'reference_pub', 'reference_lab',
            'mapping_data',  'mutant_class_id',
            'mod_date'
        ],
        'mutant_class' => [ 'symbol', 'symbol_name' ],
        'alleles'      => [
            'mutant_id',        'allele_name',
            'alt_allele_names', 'reference_lab',
            'altered_phenotype'
        ]
    );
...
sub get_loci {
    ...
    @query_output =
      ( scalar @query_output >= 10 ) ? @query_output[ 0 .. 9 ] : @query_output;
            ...
            push @query_output,
              {
                ...
                'has_alleles'   => ( scalar @mutant_allele_objs > 0 )
                ? scalar @mutant_allele_objs
                : 0,
                'mutant_class_id'     => $mutant_class_obj->mutant_class_id,
...
sub get_original_annotation {
    ...
    my ($locus_feature_obj) =
      $gff_dbh->get_features_by_name( -name => $locus, -types => 'gene' );
...
sub Bio::Graphics::Panel::create_web_map {
    ...
    foreach (@$boxes) {
        ...
        $map .=
qq(<area shape="rect" coords="$left,$top,$right,$bottom" $h $a $t/>\n);
    }
    $map .= "</map>\n";
    $map;
}
...
sub add_js_event_to_map {
    ...
    my $end5 =
      $model_feature->strand == 1 ? $model_feature->start : $model_feature->end;
    my $new_map;
...
sub get_annotation_db_features {
    ...
    my ($gff_locus_obj) =
      $gff_dbh->get_features_by_name( -name => $gene_locus, -types => 'gene' );
...
sub create_ca_model_feature {
    ...
    for my $subfeature ( @{ $ca_model_ds->{subfeatures} } ) {
        my $subfeat_obj = Bio::Graphics::Feature->new(
            ...
            -strand =>
              1,    #strand is flipped by *-1, if strand is 0, it doesn't work
